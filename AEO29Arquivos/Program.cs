...
namespace AEO29Arquivos
{
    class Program
    {
        static List<String> alunos = new List<String>();
        static Boolean AquivoExiste()
        {
            try
            {
                //alunos.AddRange(File.ReadAllLines("/home/wagner/Documentos/AEO/AEO29Arquivos/Alunos.txt"));
                alunos.AddRange(File.ReadAllLines("../../../Alunos.txt"));
                return true;
            }
            catch
            {
        ...
        static void AlterarAluno()
        {
            ...
            if (x >= 0)
            {
                ...
                // gravando alteração;
                alunos[x] = Convert.ToString(gravar.Append(y[0]).Append(";").Append(y[1]));
                Console.WriteLine("<>Precione ENTER para continuar");
                Console.ReadKey();
            }
            else if (x < 0)
            ...
            if (opcao.Length > 0)
            {
                ...
                while (imprimirMenu == true)
                {
                    ...
                    else if (opcaoEscolhida == opcao.Length)
                    {
                        ...
                        //File.WriteAllLines("/home/wagner/Documentos/AEO/AEO29Arquivos/Alunos.txt", alunos);
